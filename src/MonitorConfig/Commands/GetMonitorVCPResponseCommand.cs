            ...
            foreach (var inputMonitor in Monitor)
            {
                if (All)
                {
                    ...
                    catch (Win32Exception error)
                    {
                        ...
                        new ApiException($"Failed to enumerate the VCP features due to error: {error.Message}", error),
                        "EnumerateVCPError",
                ...
                foreach (var code in VCPCode)
                {
                    ...
                    {
                        WriteError(new ErrorRecord(
                        new ApiException($"Failed to get data for code {code} due to error: {error.Message}", error),
                        "VCPResponseError",
                        error.GetErrorCategory(),
                        inputMonitor));
                    }
